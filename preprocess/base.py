...
from preprocess import sigproc_tools as sigproc
...
if __name__ == '__main__':
    ...
    for root, dirs, files in os.walk(c.DATA_TEMP):
        for file in files:
            if not file.endswith(".wav"):
                continue
            filename = os.path.join(root, file)
            filenames.append(filename)
            soundfile = load_wav(filename, c.SAMPLE_RATE)
            feature = mfcc(soundfile,
                           samplerate=c.SAMPLE_RATE,
                           winlen=c.FRAME_LEN,
                           winstep=c.FRAME_STEP,
                           nfft=c.NUM_FFT,
                           preemph=c.PREEMPHASIS_ALPHA,
                           #winfunc=np.hamming()
                            )
            print(feature)
            print(feature.shape)