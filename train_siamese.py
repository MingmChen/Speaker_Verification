...
def train_with_loader(train_loader, n_labels, num_channels):
    ...
    for epoch in range(n_epochs):
        ...
        for i, data in enumerate(train_loader, 1):
            ...
            y = torch.eq(train_labels[0:int(c.BATCH_SIZE/2)], train_labels[int(c.BATCH_SIZE/2):c.BATCH_SIZE]).float()
            if torch.cuda.is_available():
                train_input, y = Variable(train_input.cuda()), Variable(y.cuda())
            else:
            ...
            # forward
            output1 = model(train_input[0:int(c.BATCH_SIZE/2)], development=False)
            output2 = model(train_input[int(c.BATCH_SIZE/2):c.BATCH_SIZE], development=False)
