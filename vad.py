...
# (You can also set the mode when you create the VAD, e.g. vad = webrtcvad.Vad(3)):
def main():
    ...
    for file in train_files:
        ...
        for i, segment in enumerate(segments):
            ...
            dest_path = dest_path.replace('.wav', '_{}.wav'.format(i))
            # print(' Writing %s' % (dest_path,))

            train_list.append(dest_path.replace(to_path + '/', ''))

            if not os.path.exists(os.path.dirname(dest_path)):
                os.makedirs(os.path.dirname(dest_path))

            write_wave(dest_path, segment, sample_rate)

    np.savetxt('100_speakers_100_samples_chunked_ids.txt', train_list, fmt='%s')
